...
def get_magnet_links(result):
    ...
    # 可能是截断的，先拼起来
    result = re.sub(
        r'([^0-9a-zA-Z])([0-9a-zA-Z]{5,30})[^0-9a-zA-Z]{5,30}([0-9a-zA-Z]{5,30})([^0-9a-zA-Z])', r'\1\2\3\4', result)
    # 40位和32位的磁力链接
...
def get_dupan_links(result):
    pairs = re.findall(r'\W(1\w{7})(提取|密码|：|\s)+(\w{4}|8酱)\W', result)
...
class HacgSpider(scrapy.Spider):
    ...
    def parse_index(self, response):
        ...
        if last_page.css('::text').extract_first() == '>':
            url = response.urljoin(last_page.css(
    ...
    def parse_page(self, response):
        contents = response.css(
            'div[class="entry-content"]').xpath('string(.)').extract()
        contents.extend(response.css(
            'div[class="comment-content"] p').xpath('string(.)').extract())
